...
<script>
        ...
        views: {
                ...
                },
                {
                    ...
                    options: {
                        plugins: {
                            datalabels: {
                                display: false
                            } // Turns off numbers on the line chart so it doesn't get cluttered
                        }
                    },
                    calcPill: () => `Active`
                }
            ...
            'forestry.plantations': [{
                    ...
                    generator: (idx, db) => {
                        ...
                        // Capitalize the first letter of phases for display
                    ...
                    options: {
                        ...
                        cutout: '70%' // Sleek thin doughnut ring
                    },
                    calcPill: (data) => {
                        let maxIdx = data.datasets[0].data.indexOf(Math.max(...data.datasets[0].data));
                ...
                },
                {
                    ...
                    calcPill: (data) => {
                        let total = data.datasets[0].data.reduce((a, b) => a + b, 0);
                        let fenced = data.datasets[0].data[0] || 0; // Assuming Fenced is index 0
    ...
    // =================================================================
    // --- GOOGLE MAPS LOGIC & DYNAMIC DATA OVERLAYS ---
    // =================================================================

    // Map markers mapped to EXACT database report_type strings
    ...
    // Google Maps Trick: Generate an SVG image data URL on the fly for the Emoji
    ...
    function initOverallMap() {
        ...
        overallMap = new google.maps.Map(mapEl, {
            ...
            mapTypeId: 'terrain',
            scrollwheel: false, // Wait for Ctrl+Scroll
            mapTypeControl: true,
        ...
        // Setup Heatmap Layer using your database coordinates
        const dbMapData = window.dashboardData.mapData || [];
        const heatMapDataPoints = dbMapData
            .filter(p => p.latitude && p.longitude)
            .map(p => new google.maps.LatLng(parseFloat(p.latitude), parseFloat(p.longitude)));

        // Try to load heatmap, fallback gracefully if library is missing
        try {
            ...
            console.error(
                "Heatmap visualization library not loaded. Ensure '&libraries=visualization' is in your Google Maps API script tag."
            );
        }
    ...
    function setupCtrlScroll(mapEl) {
        // Inject scroll warning dynamically
    ...
    function initAllMapLayers(dbMapData) {
        ...
        Object.keys(mapLayerDefinitions).forEach(category => {
            ...
            layers.forEach((layerDef) => {
                // Filter DB records for this exact report type
                ...
                layerRecords.forEach(record => {
                    ...
                    // Build Popup Content dynamically from your JSON structure
                    let popupDetails = '';
                    try {
                ...
                // Store array of markers for toggling
    ...
    // Dynamic UI overlay on top of the Map
    function createLegend(dbMapData) {
        ...
        Object.values(mapLayerDefinitions).flat().forEach(layerDef => {
            ...
            if (count > 0) {
                // Clicking a legend item toggles it on the map
                legendItemsHtml += `
    ...
    // Toggle Specific Marker Groups
    window.toggleMapLayer = function(layerId, element) {
        const markers = overlayMapGroups[layerId];
        if (!markers || markers.length === 0) return;

        // Check state of first marker to determine if we are turning on or off
        const isVisible = markers[0].getMap() !== null;

        markers.forEach(m => {
            m.setMap(isVisible ? null : overallMap); // Hide or Show
        });

        // Visually gray out the text in the legend so user knows it's off
        if (isVisible) {
    ...
    window.toggleHeatmap = function(element) {
        ...
        if (isHeatmapActive) {
            heatmapLayer.setMap(overallMap); // Turn on heatmap
            element.style.background = '#fee2e2'; // Light red background
            element.style.borderRadius = '4px';
            element.style.padding = '4px';

            // Optional: Hide all standard markers so they don't clash with the heatmap
            Object.values(overlayMapGroups).flat().forEach(m => m.setMap(null));
        } else {
            heatmapLayer.setMap(null); // Turn off heatmap
            element.style.background = 'transparent';

            // Restore all standard markers
            Object.values(overlayMapGroups).flat().forEach(m => m.setMap(overallMap));

            // Reset opacity on legend
    ...
    window.buildAnalyticalUI = function() {
        const container = document.getElementById('sub-tabs-container');
        // 🔥 Make sure this has window. in front of activeMainTab!
    ...
    window.renderAnalyticalCharts = function() {
        ...
        // 🔥 FIX 1: Combine Main Tab and Sub Tab to correctly target the config keys
        const viewKey = window.activeMainTab + '.' + window.activeSubTab;
        let chartsConfig = config.views[viewKey];

        console.log('Rendering charts for:', viewKey, chartsConfig);
        ...
        chartsConfig.forEach(cfg => {
            // 🔥 FIX 2: Generate the HTML for your toggles and calculation pills
            let togglesHtml = '';
        ...
        setTimeout(() => {
            chartsConfig.forEach(cfg => {
                ...
                // 🔥 FIX 3: Use the generator function if it exists, otherwise fallback to data()
                let raw = cfg.generator ? cfg.generator(0, db) : (cfg.data ? cfg.data(db) : {});

                // Flatten water logic if needed
                if (window.activeSubTab === 'water' && cfg.id === 'c1' && !raw.datasets) {
                    let flattened = {};
                ...
                // Check if generator returned a full ChartJS object or just raw arrays
                ...
                // Update the pill text on initial render
                if (cfg.calcPill) {
