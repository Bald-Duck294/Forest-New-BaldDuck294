...
@section('content')
    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex align-items-center gap-3 mb-4">
            ...
            <div>
                <h4 class="fw-bold mb-0" style="color: var(--text-main);">Edit Asset</h4>
                <p class="text-muted small mb-0">Update details for AST-{{ $asset->id }}</p>
            </div>
        </div>
            ...
            <form action="{{ route('assets.update', $asset->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" class="custom-input" value="{{ $asset->name }}" required>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="category" class="custom-input">
                            ...
                            <option {{ $asset->category == 'Offices / Govt Residence' ? 'selected' : '' }}>Offices / Govt
                                Residence</option>
                            <option {{ $asset->category == 'Nursery' ? 'selected' : '' }}>Nursery</option>
                            <option {{ $asset->category == 'Plantations' ? 'selected' : '' }}>Plantations</option>
                            <option {{ $asset->category == 'Eco Tourism Sites' ? 'selected' : '' }}>Eco Tourism Sites
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="condition" class="custom-input">
                            ...
                            <option {{ $asset->condition == 'Poor' ? 'selected' : '' }}>Poor</option>
                            <option {{ $asset->condition == 'Not in Use' ? 'selected' : '' }}>Not in Use</option>
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label class="custom-label">Year of Acquisition</label>
                        <input type="number" name="year" class="custom-input" value="{{ $asset->year }}"
                            placeholder="YYYY">
                    </div>

                    <div class="col-md-12">
                        <label class="custom-label">Description & Notes</label>
                        <textarea name="description" rows="3" class="custom-input">{{ $asset->description }}</textarea>
                    </div>

                    <div class="col-md-12">
                        <label class="custom-label">Upload Additional Photos</label>
                        <input type="file" name="photos[]" class="custom-input custom-file-input" multiple
                            accept="image/*">
                        <div class="text-muted mt-1" style="font-size: 0.75rem;">Existing photos will remain.</div>
                    </div>

                    @if ($asset->photos && is_array($asset->photos) && count($asset->photos) > 0)
                        <div class="col-md-12">
                            <label class="custom-label">Existing Photos</label>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="text-muted mt-2 text-end" style="font-size: 0.75rem;">Drag the pin to update precise
                            coordinates.</div>

                        <input type="hidden" id="lat" name="location[lat]"
                            value="{{ $asset->location['lat'] ?? '' }}">
                        <input type="hidden" id="lng" name="location[lng]"
                            value="{{ $asset->location['lng'] ?? '' }}">
                    </div>
                </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function initMap() {
                ...
                // Default to a fallback if coords are missing or invalid
