...
<div class="container py-4">
    <nav class="d-flex justify-content-between align-items-center mb-4" aria-label="breadcrumb">
        ...
        <a href="{{ route('companies.create') }}" class="btn btn-primary btn-sm px-3 rounded-3 shadow-sm d-flex align-items-center gap-2">
            ...
            <form action="{{ route('global.companies') }}" method="GET" class="row g-3 align-items-center">
                <div class="col-md-5">
                    <div class="input-group">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" class="form-control bg-body-tertiary border-0 text-body" placeholder="Search by company name...">
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-end">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('companies.edit', $company->id) }}" class="btn btn-outline-secondary border-0 bg-transparent text-secondary p-1 px-2" title="Edit">
                                    <i class="bi bi-pencil-square fs-6"></i>
                                </a>
                                <button class="btn btn-outline-danger border-0 bg-transparent p-1 px-2" title="Archive">
                                    <i class="bi bi-archive fs-6"></i>
                                </button>
                                <a href="{{ route('global.enter_simulation', $company->id) }}" class="btn btn-sm btn-outline-primary border-0 bg-primary bg-opacity-10">
