    ...
    function showModal() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // ✅ Bootstrap 5 correct way
                var modal = new bootstrap.Modal(document.getElementById('viewModal'));
                modal.show();

                if (!response || response == 'error') {
                    document.getElementById("siteWiseGuardReports").innerHTML =
                        "<div style='padding:20px;text-align:center;'>No data found</div>";
                } else {
            ...
            error: function(err) {
                ...
                // 1. Hide the infinite loading spinner
                $('#loader').hide();

                // 2. Initialize and show the modal
                var myModal = bootstrap.Modal.getOrCreateInstance(document.getElementById('viewModal'));
                myModal.show();

                // 3. Inject the "No data found" message into the modal body
                document.getElementById("siteWiseGuardReports").innerHTML =
                    `<div class="d-flex flex-column align-items-center justify-content-center py-5">
                        <i class="bi bi-folder-x text-muted mb-3" style="font-size: 3rem;"></i>
                        <h5 class="text-muted fw-normal">No data found</h5>
                        <p class="text-muted small">We couldn't find any records for the selected filters or the server encountered an issue.</p>
                     </div>`;
            }
        });
    }
