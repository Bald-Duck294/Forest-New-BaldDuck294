...
@section('content')
    ...
    <div class="container-fluid py-4">
        ...
        <div class="dash-card p-4 mb-4">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    <h3 class="fw-bold mb-1" style="color: var(--text-main);">Advanced Analytics</h3>
                        ...
                        <div class="col-md-5">
                            ...
                            <select id="playbackSession" class="custom-input">
                                ...
                                @foreach ($rawSessions as $s)
                                    <option value="{{ $s['id'] }}">Session #{{ $s['id'] }} (User
                                        {{ $s['user_id'] }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-7 d-flex gap-2">
                            <button id="playBtn" class="btn-sapphire btn-sm px-3"><i class="bi bi-play-circle"></i>
                                Play</button>
                            <button id="pauseBtn" class="btn-sapphire-outline btn-sm px-3"><i
                                    class="bi bi-pause-circle"></i> Pause</button>
                            <button id="resetBtn" class="btn btn-sm"
                                style="background: var(--sapphire-danger); color: white;"><i
            ...
            <div class="col-lg-6">
                <div class="dash-card p-0 overflow-hidden h-100">
                    <div class="p-3"
                        ...
                        <h5 class="fw-bold mb-0" style="color: var(--sapphire-success);"><i
                                class="bi bi-bullseye me-2"></i> Patrol Coverage Scoring</h5>
                    </div>
    ...
    <script>
        ...
        function renderAllCharts() {
            ...
            const weeklyLabels = weeklyStats.map(w => "Week " + w.week_number);
            const weeklyScores = weeklyStats.map(w => w.score);
        ...
        window.initMap = function() {
            ...
            // Heatmap
            const heatData = heatmapPoints.map(p => new google.maps.LatLng(p.lat, p.lng));
        ...
        function drawSessionsAndCoverage() {
            ...
            rawSessions.forEach(s => {
                ...
                if (siteFence && siteFence.coords) {
                    ...
                    const cov = Math.floor(Math.random() * 100);
                    ...
                    htmlTable +=
                        `<tr><td class="ps-4">#${s.id}</td><td>${siteLabels[s.site_id] ?? s.site_id}</td><td><span class="badge-soft badge-soft-success">${cov}%</span></td></tr>`;
                }
            });
