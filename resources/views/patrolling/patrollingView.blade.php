...
@section('title', 'Patrolling Status')
...
@section('content')
    ...
    <style>
        ...
        /* =========================================
                ...
                ========================================= */
        /* Hide native multi-select box if Select2 takes a second to load */
        select[multiple] {
            max-height: 42px;
            overflow: hidden;
        }
        ...
        /* Interactive Hover Lift for Grid Cards */
        .hover-lift {
    ...
    <div class="container-fluid py-4">
        ...
        <div class="dash-card mb-4 p-0 overflow-visible">
            ...
            {{-- Filters Body --}}
            <div class="p-4">
                <form method="GET" action="{{ route('patrolling') }}" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        <button type="submit" class="btn-sapphire flex-grow-1">
                            <i class="bi bi-funnel"></i> Apply
                        </button>
                        <a href="{{ route('patrolling') }}" class="btn-sapphire-outline flex-grow-1" title="Reset Filters">
            ...
            <div class="row g-3">
                    ...
                    <div class="col-12 col-md-6 col-xl-4">
                        <div class="dash-card hover-lift h-100 d-flex flex-column p-4">
                            ...
                            <div class="flex-grow-1 p-3 rounded mb-3"
                                ...
                                <div class="d-flex justify-content-between mb-2">
                                    <small
                                        style="color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; font-weight: 600;">Method</small>
                                    <span class="badge-soft badge-soft-primary"
                                        style="padding: 2px 8px;">{{ ucfirst($patrol->method) ?? 'N/A' }}</span>
                                </div>
                                <div class="d-flex justify-content-between mb-2">
                                    <small
                                        style="color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; font-weight: 600;">Start</small>
                                    <span
                                        style="color: var(--text-main); font-size: 0.85rem;">{{ $patrol->started_at ? \Carbon\Carbon::parse($patrol->started_at)->format('d M, Y h:i A') : 'N/A' }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <small
                                        style="color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; font-weight: 600;">End</small>
                                    <span
                                        style="color: var(--text-main); font-size: 0.85rem;">{{ $patrol->ended_at ? \Carbon\Carbon::parse($patrol->ended_at)->format('d M, Y h:i A') : 'N/A' }}</span>
                                </div>
                            </div>

                            {{-- Card Footer --}}
                            <div class="mt-auto pt-2" style="border-top: 1px dashed var(--border-color);">
                                <a href="{{ route('patrolling.details', $patrol->id) }}"
                                    class="btn-sapphire-outline w-100 d-block text-center text-decoration-none"
                ...
                @empty
                    <div class="col-12 text-center py-5">
                        ...
                        <h5 style="color: var(--text-muted);">No patrol records found.</h5>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div id="tableView" class="view-section">
            <div class="dash-card p-0 overflow-hidden">
                ...
                <div class="d-flex justify-content-between align-items-center p-3"
                    ...
                    <form method="GET" action="{{ route('patrolling') }}" class="d-flex align-items-center gap-2 m-0">
                            ...
                            @forelse($patrols as $patrol)
                                <tr>
                                    <td class="ps-4 fw-semibold" style="color: var(--text-muted);">
                                        {{ ($patrols->currentPage() - 1) * $patrols->perPage() + $loop->iteration }}
                                    </td>
                                    <td>
                                        <div class="fw-semibold" style="color: var(--text-main);">
                                            {{ $patrol->user->name ?? 'N/A' }}</div>
                                    </td>
                                    <td style="color: var(--text-main);">{{ $patrol->site->client->name ?? 'N/A' }}</td>
                                    <td style="color: var(--text-main);">{{ $patrol->site->name ?? 'N/A' }}</td>
                                    <td>
                                        <span class="badge-soft badge-soft-primary">
                                            {{ ucfirst($patrol->method) ?? 'N/A' }}
                                        </span>
                                    </td>
                                    <td style="color: var(--text-muted); font-size: 0.85rem;">
                                        {{ $patrol->started_at ? \Carbon\Carbon::parse($patrol->started_at)->format('d M, Y h:i A') : 'N/A' }}
                                    </td>
                                    <td style="color: var(--text-muted); font-size: 0.85rem;">
                                        {{ $patrol->ended_at ? \Carbon\Carbon::parse($patrol->ended_at)->format('d M, Y h:i A') : 'N/A' }}
                                    </td>
                                    <td>
                                        @if ($patrol->ended_at)
                                            <span class="badge-soft badge-soft-success">Completed</span>
        ...
        <div class="mt-4 px-2 d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
            <div>
                <small style="color: var(--text-muted);">
                    Showing {{ $patrols->firstItem() ?? 0 }} to {{ $patrols->lastItem() ?? 0 }} of
                    {{ $patrols->total() ?? 0 }} results
                </small>
            </div>
            <div>
                {{ $patrols->appends(request()->query())->links('pagination::bootstrap-5') }}
            </div>
        </div>
        ...
        <script>
            ...
            function setView(viewType) {
                ...
                if (viewType === 'table') {
                    ...
                    localStorage.setItem('patrolStatusView', 'table');
                } else {
                    tableView.classList.remove('active');
                    gridView.classList.add('active');
                    btnGrid.classList.add('active');
                    btnTable.classList.remove('active');
                    localStorage.setItem('patrolStatusView', 'grid');
                }
            }
            ...
            $(document).ready(function() {
                ...
                const savedView = localStorage.getItem('patrolStatusView') || 'table';
                setView(savedView);
