...
@section('title', get_label('label_client', 'Clients'))
    ...
    <div class="dash-card p-0 overflow-hidden">
        ...
        <div class="p-3 p-md-4 d-flex flex-column flex-xl-row justify-content-between align-items-xl-center gap-3 border-bottom"
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h4 class="fw-bold mb-0" style="color: var(--text-main); line-height: 1.2;">
                        Manage {{ get_label('label_client', 'Clients') }}
                    </h4>
            ...
            <div class="d-flex flex-column flex-md-row gap-2">
                ...
                <a href="{{ route('clients.create') }}" class="btn-sapphire text-nowrap">
                    <i class="bi bi-plus-lg"></i> Add {{ get_label('label_client', 'Client') }}
                </a>
            </div>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        window.deleteClient = function(id) {
            var msg = 'Are you sure you want to delete this {{ strtolower(get_label("label_client", "client")) }}?';
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // REMOVED SPACES FROM ROUTE CALL
        ...
        window.toggleActive = function(id, element, currentlyActive) {
            ...
            var msg = `Are you sure you want to ${action} this {{ strtolower(get_label("label_client", "client")) }}?`;
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // REMOVED SPACES FROM ROUTE CALLS
                        var url = currentlyActive ? '{{ route("clients.inactive", ":id") }}' : '{{ route("clients.active", ":id") }}';
                        window.location = url.replace(':id', id);
                    } else {
