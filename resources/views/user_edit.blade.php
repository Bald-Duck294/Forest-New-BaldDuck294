    ...
    <div class="main-content-wrapper">
        ...
        <div class="edit-card">
            <div class="edit-card-header">
                <a href="javascript:history.back()" class="text-muted" style="text-decoration: none; font-size: 1.2rem;">
                    <i class="bi bi-arrow-left"></i>
                </a>
                <h4>
                    <i class="bi bi-person-gear text-primary"></i>
                    Edit User: <span class="text-primary ml-1">{{ $editUser->name }}</span>
                </h4>
            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                                ...
                                <input type="text" autocomplete="off" class="form-control" id="name" name="name"
                                    placeholder="Enter Full Name" value="{{ old('name', $editUser->name) }}">
                                @if ($errors->has('name'))
                            ...
                            <div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control" name="gender" id="gender">
                                    <option value="" disabled
                                        {{ old('gender', $editUser->gender) == '' ? 'selected' : '' }}>Select Gender
                                    </option>
                                    <option value="Male"
                                        {{ old('gender', $editUser->gender) == 'Male' ? 'selected' : '' }}>Male</option>
                                    <option value="Female"
                                        {{ old('gender', $editUser->gender) == 'Female' ? 'selected' : '' }}>Female
                                    </option>
                                    <option value="Other"
                                        {{ old('gender', $editUser->gender) == 'Other' ? 'selected' : '' }}>Other</option>
                                </select>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                                ...
                                <textarea name="address" class="form-control" id="address" placeholder="Enter full address..." rows="2">{{ old('address', $editUser->address) }}</textarea>
                                ...
                                <select class="form-control" name="role_id" id="role_id">
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->id }}"
                                            {{ old('role_id', $editUser->role_id) == $role->id ? 'selected' : '' }}>
                                            {{ ucfirst($role->role_name) }}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('role_id'))
                                    <span class="text-danger small mt-1 d-block">{{ $errors->first('role_id') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
