...
<div id="loader" class="fixed-top w-100 h-100 bg-white bg-opacity-75 d-none flex-column align-items-center justify-content-center" style="z-index: 9999;">
    <i class="bi bi-leaf text-success animate-float" style="font-size: 3rem;"></i>
    <h5 class="mt-3 fw-bold">Refreshing Analytics...</h5>
</div>
...
<main class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <!-- <div class="d-flex align-items-center gap-2">
            <div class="bg-success p-2 rounded text-white shadow-sm"><i class="bi bi-leaf"></i></div>
            <div>
                <h4 class="fw-bold mb-0">ForestFix</h4>
                <small class="text-muted fw-bold" style="font-size: 10px; letter-spacing: 1px;">PROTECTION ANALYTICS</small>
            </div>
        </div> -->

        <div class="d-flex gap-2">
            {{-- 1. Range Dropdown - Safety Check Added --}}
            <select class="form-select form-select-sm border-0 shadow-sm" style="width: 150px;">
                <option value="">All Ranges</option>
                @if(isset($ranges) && count($ranges) > 0)
                @foreach($ranges as $range)
                @if(is_object($range))
                <option value="{{ $range->id ?? '' }}">{{ $range->name ?? 'Unnamed Range' }}</option>
                @endif
                @endforeach
                @endif
            </select>

            {{-- 2. Beat Dropdown - Safety Check Added --}}
            <select class="form-select form-select-sm border-0 shadow-sm" style="width: 150px;">
                <option value="">All Beats</option>
                @if(isset($beats) && count($beats) > 0)
                @foreach($beats as $beat)
                @if(is_object($beat))
                <option value="{{ $beat->id ?? '' }}">{{ $beat->name ?? 'Unnamed Beat' }}</option>
                @endif
                @endforeach
                @endif
            </select>

            <button onclick="refreshData()" class="btn btn-sm btn-light border shadow-sm text-success" title="Sync Data">
                <i class="bi bi-arrow-clockwise"></i>
            </button>
        </div>
    </div>

    {{-- 2. DYNAMIC 6-COLUMN KPI GRID --}}
    <div id="main-kpi-grid" class="row g-3 mb-4">
        @php
        $items = [
        ['id' => 'officers', 'label' => 'On Duty Officers', 'val' => $kpis['officers'] ?? 0, 'icon' => 'people', 'color' => 'blue-light', 'trend' => 'Registered Staff'],
        ['id' => 'patrol', 'label' => 'Patrol Status', 'val' => $kpis['patrols'] ?? 0, 'icon' => 'map', 'color' => 'indigo-light', 'trend' => 'Active Reports'],
        ['id' => 'criminal', 'label' => 'Criminal Activity', 'val' => $kpis['criminal'] ?? 0, 'icon' => 'exclamation-triangle', 'color' => 'rose-light', 'nav' => 'criminal'],
        ['id' => 'events', 'label' => 'Events & Monitoring', 'val' => $kpis['events'] ?? 0, 'icon' => 'binoculars', 'color' => 'emerald-light', 'nav' => 'events'],
        ['id' => 'fire', 'label' => 'Fire Alerts', 'val' => $kpis['fire'] ?? 0, 'icon' => 'fire', 'color' => 'orange-light', 'nav' => 'fire'],
        ['id' => 'assets', 'label' => 'Assets & Tools', 'val' => $kpis['assets'] ?? 0, 'icon' => 'shield-check', 'color' => 'teal-light', 'nav' => 'assets'],
        ];
        @endphp

        @foreach($items as $item)
        <div class="col-6 col-md-4 col-lg-2">
            <div class="card h-100 border-0 shadow-sm p-3 kpi-card"
                onclick="navigateTo('{{ $item['nav'] ?? '' }}')">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div>
                        <p class="text-muted mb-1" style="font-size: 11px; font-weight: 600;">{{ strtoupper($item['label']) }}</p>
                        <h3 class="fw-bold mb-0 text-dark" id="kpi-{{ $item['id'] }}">{{ number_format($item['val']) }}</h3>
                    </div>
                    <div class="icon-box-refined bg-{{ $item['color'] }}">
                        <i class="bi bi-{{ $item['icon'] }}"></i>
                    </div>
                </div>
                <div class="mt-2">
                    <p class="mb-0" style="font-size: 10px; font-weight: 600; color: #64748b;">
                        @isset($item['nav'])
                        <span class="text-success">Click for details <i class="bi bi-arrow-right ms-1"></i></span>
                        @else
                        <i class="bi bi-check-circle-fill me-1 text-success"></i> {{ $item['trend'] }}
                        @endisset
                    </p>
                </div>
            </div>
    ...
    <div class="d-flex mb-3">
        <div class="bg-white p-1 rounded-3 shadow-sm border d-inline-flex">
            <button id="view-overall" onclick="setViewMode('overall')" class="btn btn-sm px-3 tab-btn active">Overall View (Map)</button>
            <button id="view-analytical" onclick="setViewMode('analytical')" class="btn btn-sm px-3 tab-btn inactive">Analytical View</button>
        </div>
    </div>

    {{-- 4. OVERALL VIEW --}}
    <div id="overall-container" class="row g-3">
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm position-relative overflow-hidden" style="height: 500px; border-radius: 15px; z-index: 1;">
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm h-100 p-4" style="border-radius: 15px;">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h6 class="fw-bold mb-0">Territory Overview</h6>
    ...
    {{-- 5. ANALYTICAL VIEW --}}
    <div id="analytical-container" class="d-none mt-4">
        <div class="d-flex gap-5 border-bottom mb-4 px-2 overflow-auto hide-scrollbar" id="main-tabs-nav"></div>
        <div id="breakdown-header" class="mb-4">
            <h4 id="breakdown-title" class="fw-bold mb-1">Breakdown</h4>
            <p class="text-muted small">Detailed view of selected category status</p>
        </div>
        <div id="sub-tabs-container" class="d-flex gap-3 overflow-auto hide-scrollbar mb-5 pb-2"></div>
...
<script>
    ...
    function setViewMode(mode) {
        ...
        if (analyticalBtn) analyticalBtn.className = `btn btn-sm px-3 tab-btn ${mode==='analytical'?'active':'inactive'}`;
    ...
    function renderAnalyticalCharts() {
        ...
        setTimeout(() => {
            ...
            activeCharts.c2 = new Chart(document.getElementById('chart2'), {
                type: (activeSubTab === 'felling' || activeSubTab === 'compensation') ? 'pie' : 'doughnut',
                data: {
                    labels: currentLabels,
                    datasets: [{
                        data: currentValues,
