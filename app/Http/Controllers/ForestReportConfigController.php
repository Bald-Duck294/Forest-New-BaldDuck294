...
class ForestReportConfigController extends Controller
{
    ...
    //             // 🔥 SMART PARSE: Use Vehicle Type as the "Method"
    //             $isSeized = $data['vehicle_seized'] ?? 'No';
    //             if (strtolower($isSeized) === 'yes') {
    //                 $vehRaw = $data['seized_vehicle_type'] ?? 'Unknown';
    //                 if (strtolower(trim($vehRaw)) === 'other' && !empty($data['seized_vehicle_type_other'])) {
    //                     $vehRaw = $data['seized_vehicle_type_other'];
    //                 }
    //                 $method = empty(trim($vehRaw)) ? 'Unknown Vehicle' : trim($vehRaw);
    //             } else {
    //                 $method = 'Manual / Not Seized';
    //             }

    //             $analytics['mining']['minerals'][$minType] = ($analytics['mining']['minerals'][$minType] ?? 0) + 1;

    //             // Add to the 'methods' array so the JS chart picks it up
    //             $analytics['mining']['methods'][$method] = ($analytics['mining']['methods'][$method] ?? 0) + $vol;
    ...
    public function reportsDashboard(Request $request)
    {
        ...
        // =======================================================================
        // 1. BUILD BASE QUERIES FOR REPORTS, ASSETS & PLANTATIONS
        // =======================================================================
        $query = DB::table('forest_reports')->where('company_id', $companyId);
        $assetQuery = Asset::where('company_id', $companyId);
        ...
        // =======================================================================
        // 2. STRICT "TODAY" ATTENDANCE LOGIC (Ignores Global Date Filter completely)
        // =======================================================================
        $totalGuards = DB::table('users')->where('company_id', $companyId)->where('role_id', 3)->where('showUser', 1)->count();

        $attendanceQuery = DB::table('attendance')
            ->where('company_id', $companyId)
            ->where('role_id', 3)
            ->whereDate('dateFormat', \Carbon\Carbon::today()); // ALWAYS EXACTLY TODAY
        ...
        // ✅ FINAL COUNT FOR ON DUTY OFFICERS (Uses DISTINCT so double check-ins count as 1)
        $activeOfficersCount = $attendanceQuery->distinct('user_id')->count('user_id');
        $attendanceRate = $totalGuards > 0 ? round(($activeOfficersCount / $totalGuards) * 100) : 0;


        // =======================================================================
        // 3. FETCH REAL RANGES & BEATS FOR DROPDOWNS
        // =======================================================================
        $ranges = DB::table('client_details')->where('company_id', $companyId)->pluck('name', 'id');
        $beats = DB::table('site_details')->where('company_id', $companyId)->select('id', 'name', 'client_id')->get();
        ...
        $deploymentTrend = (clone $assetQuery)->select(DB::raw('WEEK(created_at) as week'), DB::raw('count(*) as total'))
            ->groupBy('week')->orderBy('week')->get()->mapWithKeys(function ($item) {
                return ["Wk " . $item->week => $item->total];
            })->toArray();
        ...
        // 🔥 MAP DATA REVERSE LOOKUP + JFMC OVERRIDE
        $enhancedMapData = $allReports->whereNotNull('latitude')->whereNotNull('longitude')->map(function ($report) use ($rangesMap, $beatsMap) {
            $parsedData = is_string($report->report_data) ? json_decode($report->report_data, true) : [];
            $siteId = $report->site_id ?? $parsedData['site_id'] ?? null;
            $clientId = $report->client_id ?? $parsedData['client_id'] ?? null;
            ...
            if (strtolower(trim($report->report_type)) === 'jfmc') {
                $report->category = 'crimes';
            }
