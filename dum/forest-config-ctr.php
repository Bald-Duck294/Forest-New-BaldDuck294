...
class ForestReportConfigController extends Controller
{
    ...
    public function reportsDashboard(Request $request)
    {
        ...
        // 🔥 NEW: Calculate Late and Absent for today
        $lateOfficersCount = (clone $attendanceQuery)
            ->whereNotNull('attendance.lateTime')
            ->distinct('attendance.user_id')
            ->count('attendance.user_id');

        $onTimeCount = max(0, $activeOfficersCount - $lateOfficersCount);
        $absentCount = max(0, $totalGuards - $activeOfficersCount);

        // 🔥 NEW: 7-Day Attendance Trend
        $weeklyTrend = [];
        for ($i = 6; $i >= 0; $i--) {
            $d = \Carbon\Carbon::today()->subDays($i)->format('Y-m-d');
            $dayCount = DB::table('attendance')
                ->where('company_id', $companyId)
                ->where('role_id', 3)
                ->where('dateFormat', $d)
                ->distinct('user_id')
                ->count('user_id');
            $weeklyTrend[\Carbon\Carbon::parse($d)->format('M d')] = $dayCount;
        }
        ...
        $stats = (object) [
            ...
            'jfmc' => $allReports->where('report_type', 'jfmc')->count(), // 🔥 NEW
        ];
        ...
        $analytics = [
            ...
            'jfmc' => ['villages' => [], 'ranges' => [], 'trend' => []], // 🔥 NEW
            ...
            'attendance' => [
                'today' => ['Present' => $onTimeCount, 'Late' => $lateOfficersCount, 'Absent' => $absentCount],
                'punctuality' => ['On Time' => $onTimeCount, 'Late' => $lateOfficersCount],
                'trend' => $weeklyTrend
            ]
        ];
